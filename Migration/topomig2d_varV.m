...
if flag==1
    ...
    topo=[zeros(1,round((width/2/dx))) topo' zeros(1,round((width/2/dx)))];
    ...
    for j=los:bis-1 % go through traces (without padded area)
        ...
        ind_z=find(abs(z-topo(j))==min(abs(z-topo(j))),1,'first'); % get z-index of surface point in this trace
