% Script for reading Mala-datafiles (NOT in rSlicer folder but all channels individually)
% and processing them (no binning!)
%
% Dr. Tina Wunderlich, CAU Kiel 2024, tina.wunderlich@ifg.uni-kiel.de
%
% requires following files:
% *.rd7: Radar data of individual channels
% *.rad: header file
% *.pos: positioning file with position for start and end point of the
% profile (same format as the "normal" pos-files in the rSlicer-folder, but
% just with one coordinate for first and last trace).
%
...
if userawdata==0  % first run of program -> read all profiles
    ...
    for i=1:lnum
        ...
        [traces,dt,ns,tempx{i},tempy{i},tempz{i},channels]=readmala_withoutGPS(foldername,name,numbers(i));
