...
%     % check again for correct coordinates without jumps/strong turns:
%     d1=diff(xn);
%     d2=diff(yn);
%     d1=[d1(:); d1(end)];
%     d2=[d2(:); d2(end)];
%     num3=round(length(d1)/3); % number of points for about 1/3 of profile
%     diffe = sqrt(d1.^2 +d2.^2); % distance between adjacent points
%     angle=atan2d(d2,d1);
%     diffangle=[diff(angle); 0]; % difference in angles
%     % start from 1/3 or profile towards beginning to find coordinate jumps
%     % and angle jumps:
%     jumpstart=num3-find(flipud(diffe(1:num3))>5*mean(diffe),1,'first'); % find coordinate jumps
%     anglestart=num3-find(flipud(diffangle(1:num3))>50*mean(diffangle),1,'first'); % find large angle jumps
%     % start from 2/3 of profile towards the end:
%     jumpend=2*num3+find(diffe(2*num3:end)>3*mean(diffe),1,'first');
%     angleend=2*num3+find(diffangle(2*num3:end)>10*mean(diffangle),1,'first');
%     if isempty(jumpstart)
%         jumpstart=1;
%     end
%     if isempty(anglestart)
%         anglestart=1;
%     end
%     if isempty(jumpend)
%         jumpend=length(d1);
...
%     end
% 
%     deltraces2=zeros(size(diffe));
%     deltraces2(1:max(jumpstart,anglestart))=1; % delete all trace before coordinate/angle jump
%     deltraces2(min(jumpend,angleend)-1:end)=1; % delete all traces after the coordinate/angle jump
%     % also mark these traces in deltraces:
%     ind=1:length(deltraces);
%     ind2=ind(deltraces==0); % indices of traces that are ok from beginning
%     deltraces(ind2)=deltraces2; % replace these zeros with logical vector deltraces2
%     % delete these traces in coordinates:
%     xn=xn(~deltraces2);
%     yn=yn(~deltraces2);
%     zn=zn(~deltraces2);
% 
%     % correct all fields in trh:
%     for i=1:length(fn)
%         trh.(fn{i})=trh.(fn{i})(~deltraces2);
%     end
%     % also delete traces in data:
%     data=data(:,~deltraces2);
