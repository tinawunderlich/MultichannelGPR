function []=write_geoPNGW_radargrams(x,y,coordtrans,filename)

%%% []=write_geoPNGW_radargrams(x,y,coordtrans,filename) 
%
% Dr. Tina Wunderlich, CAU Kiel 2020, tina.wunderlich@ifg.uni-kiel.de
%
% USE FOR RADARGRAMS ONLY! t0 is along profile coordinates!
%
% Input:
...
% global coordinates of upper left and upper right pixel
pix_ol=helmert([min(x(:)) min(y(:))],coordtrans(:,1:2),coordtrans(:,3:4));
pix_or=helmert([max(x(:)) min(y(:))],coordtrans(:,1:2),coordtrans(:,3:4));
alpha=atand(abs(pix_ol(2)-pix_or(2))/abs(pix_ol(1)-pix_or(1))); % angle against west
if pix_ol(2)>pix_or(2)
    alpha=-alpha;
end
% pixel lengths in all directions
A=dx*cosd(alpha);
D=dx*sind(alpha);
E=dy*cosd(alpha);
B=dy*sind(alpha);
