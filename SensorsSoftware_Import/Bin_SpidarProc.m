% Script for reading processed Spidar-datafiles in profiles2mat/proc and
...
% set):  Subfunctions
%
% Choose folder with original data files when asked.
...
if ispc
    if exist('temp.temp') % read last opened folder from temp.temp
        ...
        fclose(fid);
        if ~isempty(fn{1})
            foldername=uigetdir(fn{1}{1},'Choose folder with original data files');
        else
            foldername=uigetdir([],'Choose folder with original data files');
        end
        ...
        foldername=uigetdir([],'Choose folder with original data files'); % path to radargram-folder
...
for i=1:num_xrect
    for j=1:num_yrect
        ...
        z=griddata(xylist(ind_in & xylist(:,5)==round(channels/2),2),xylist(ind_in & xylist(:,5)==round(channels/2),3),xylist(ind_in & xylist(:,5)==round(channels/2),4),x,y);
