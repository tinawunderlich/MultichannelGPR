...
if ~exist(fullfile(foldername,settings),'file') % if no settings-file is found: create default file
    ...
    fprintf(fid,'do_traceInterpolation 2\nminfactor 3\nmaxfactor 3\n\n');
...
if useprocdata==0
    ...
    for i=1:length(numbers) % (is not working as parfor!)
        ...
        if exist(fullfile(foldername,'profiles2mat',[name,'_',int2str(numbers(i)),'.mat']),'file')
            ...
            % processing:
            [traces,ns,t,zmig]=processing(steps,order,traces,info,t,params,fhProc,b);
            ...
            if (continuedata==1 && continuerectangle<=anz) || continuedata==0
                ...
                if pfw==0
                    for n=1:length(nn) % parallel loop over profiles in rectangle
                        ...
                        m=matfile(fullfile(foldername,'profiles2mat',[name,'_',int2str(nn(n)),'_info.mat']));
...
%--------------------------------------------------------------------------
%%% subfunctions:
function [datatraces,ns,t,zmig]=processing(steps,order,datatraces,info,t,params,fh,b)
