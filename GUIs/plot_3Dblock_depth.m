function [] = plot_3Dblock_depth(data,xgrid,ygrid,zgrid,pfad)

% plot_3Dblock_depth(data,xgrid,ygrid,tgrid,pfad)
%
% GUI for plotting of 3D block in depth domain (e.g. for migrated data)
%
...
% zgrid: depth [m], same size as xgrid/ygrid with depth along third dimension
% pfad: complete path 
...
S.y = ygrid;
S.z = zgrid;
S.dz=abs(S.z(1,1,2)-S.z(1,1,1));
S.data=data;
S.dx=xgrid(1,2,1)-xgrid(1,1,1);

% calculate gained data
gainf=[-20 0 15 25 30];
g=interp1(linspace(0,max(S.z(:)),length(gainf)),gainf,S.z);
...
S.fh.Visible='on';
    ...
    function S=addcomponents(S)
        ...
        S.zloc=S.z(1,1,round(length(S.z(1,1,:))/2)); % z-location of slice
        % Plot slices:
        surf(permute(S.x(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.y(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.z(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.datagain(:,S.x(1,:,1)==S.xloc,:),[1 3 2]))
        hold on
        surf(permute(S.x(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.y(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.z(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.datagain(S.y(:,1,1)==S.yloc,:,:),[2 3 1]))
        surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.datagain(:,:,S.z(1,1,:)==S.zloc))
        shading interp
        plot3([S.xloc S.xloc],[S.yloc S.yloc],[0 S.zloc],'k') % vertical crossing line of slices
        plot3([S.xloc S.xloc],[min(S.y(:,1,1)) max(S.y(:,1,1))],[S.zloc S.zloc],'k') % horizontal line 1
        plot3([min(S.x(1,:,1)) max(S.x(1,:,1))],[S.yloc S.yloc],[S.zloc S.zloc],'k') % horizontal line 2
        colorbar('east');
        ...
        zlabel('z [m]')
        axis xy
        set(S.ax,'DataAspectratio',[1 1 1],'Zdir','normal')
        grid on
        axis tight
        set(S.ax,'ZLim',[min(S.z(:)) max(S.z(:))])
        ...
        S.slt = uicontrol(S.fh,'style','slide',...
            ...
            'min',min(S.z(1,1,:)),'max',max(S.z(1,1,:)),'val',S.zloc,...
            'sliderstep',[S.dz/(max(S.z(1,1,:))-min(S.z(1,1,:))) 1/(max(S.z(1,1,:))-min(S.z(1,1,:)))],'Callback',@slt_callback);
        S.cb_t = uicontrol(S.fh,'style','checkbox',...
            'unit','pix','Value',1,...
            'position',[330 80 150 20],'String','z-location of slice [m]','Callback',@cbt_call);
        S.text_tloc=uicontrol(S.fh,'style','edit','unit','pix','position',[330 55 100 20],'String',num2str(S.zloc,2),'Callback',@tloc_edit);
        
        
        % UI control aspectratio
        S.asp_String=[{'20/1'} {'10/1'} {'5/1'} {'1/1'} {'1/5'} {'1/10'} {'1/20'} {'1/30'} ];
        S.aspval=1/1;
        S.asp = uicontrol(S.fh,'style','listbox','unit','pix','position',[30 150 60 60],'String',S.asp_String,'Value',4,'Callback',@asp_call);
        S.asptext=uicontrol(S.fh,'style','text','unit','pix','position',[30 215 80 20],'String','Aspect ratio z','HorizontalAlignment','left');
...
%%%---------- Callback functions ----------------
    ...
    function [] = save_call(varargin)
        ...
        if S.cb_x.Value==1 && S.cb_y.Value==1 && S.cb_t.Value==1            % X Y Z
            if S.gain_cb.Value==1
                ...
                surf(permute(S.x(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.y(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.z(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.data(:,S.x(1,:,1)==S.xloc,:),[1 3 2]))
                hold on
                surf(permute(S.x(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.y(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.z(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.data(S.y(:,1,1)==S.yloc,:,:),[2 3 1]))
                surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.data(:,:,S.z(1,1,:)==S.zloc))
            end
            shading interp
            plot3([S.xloc S.xloc],[S.yloc S.yloc],[0 max(S.z(1,1,:))],'r') % vertical crossing line of slices
            plot3([S.xloc S.xloc],[min(S.y(:,1,1)) max(S.y(:,1,1))],[S.zloc S.zloc],'r') % horizontal line 1
            plot3([min(S.x(1,:,1)) max(S.x(1,:,1))],[S.yloc S.yloc],[S.zloc S.zloc],'r') % horizontal line 2
            % name for saving
            n1=['x',num2str(S.xloc,4),'m_y',num2str(S.yloc,4),'m_z',num2str(S.zloc,4),'m'];
        ...
        elseif S.cb_x.Value==0 && S.cb_y.Value==1 && S.cb_t.Value==1        % Y Z
            if S.gain_cb.Value==1
                hold on
                surf(permute(S.x(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.y(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.z(S.y(:,1,1)==S.yloc,:,:),[2 3 1]),permute(S.datagain(S.y(:,1,1)==S.yloc,:,:),[2 3 1]))
                surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.datagain(:,:,S.z(1,1,:)==S.zloc))
            ...
            plot3([min(S.x(1,:,1)) max(S.x(1,:,1))],[S.yloc S.yloc],[S.zloc S.zloc],'r') % horizontal line 2
            % name for saving
            n1=['y',num2str(S.yloc,4),'m_z',num2str(S.zloc,4),'m'];
        elseif S.cb_x.Value==0 && S.cb_y.Value==0 && S.cb_t.Value==1        % Z
            if S.gain_cb.Value==1
                surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.datagain(:,:,S.z(1,1,:)==S.zloc))
            else
                surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.data(:,:,S.z(1,1,:)==S.zloc))
            ...
            shading interp
            % name for saving
            n1=['z',num2str(S.zloc,4),'m'];
        elseif S.cb_x.Value==1 && S.cb_y.Value==0 && S.cb_t.Value==1        % X Z
            if S.gain_cb.Value==1
                surf(permute(S.x(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.y(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.z(:,S.x(1,:,1)==S.xloc,:),[1 3 2]),permute(S.datagain(:,S.x(1,:,1)==S.xloc,:),[1 3 2]))
                hold on
                surf(S.x(:,:,S.z(1,1,:)==S.zloc),S.y(:,:,S.z(1,1,:)==S.zloc),S.z(:,:,S.z(1,1,:)==S.zloc),S.datagain(:,:,S.z(1,1,:)==S.zloc))
            ...
            plot3([S.xloc S.xloc],[min(S.y(:,1,1)) max(S.y(:,1,1))],[S.zloc S.zloc],'r') % horizontal line 1
            % name for saving
            n1=['x',num2str(S.xloc,4),'m_z',num2str(S.zloc,4),'m'];
        end
        colormap(flipud(gray));
        xlabel('x [m]')
        ylabel('y [m]')
        zlabel('z [m]')
        axis xy
        set(gca,'DataAspectratio',[1 1 1/S.aspval],'Zdir','normal')
        grid on
        axis tight
        set(gca,'ZLim',[min(S.z(:)) max(S.z(:))],'Fontsize',20)
        % set colorscale
        if S.gain_cb.Value==1
            if S.flag_cs==1
        ...
        elseif S.flag==2 % xt plane
            view(0, 0);
            n2='xz'; % name for saving
        elseif S.flag==3 % yt plane
            view(90, 0);
            n2='yz'; % name for saving
        elseif S.flag==4 % Dsl
            view(90, 90);
            n2='Dsl'; % name for saving
        end
        % save figure
    ...
    function [] = plotnew(varargin)
        ...
        if S.cb_x.Value==1 && S.cb_y.Value==1 && S.cb_t.Value==1            % X Y Z
            ...
            shading interp
            plot3([S.xloc S.xloc],[S.yloc S.yloc],[0 max(S.z(1,1,:))],'k') % vertical crossing line of slices
            plot3([S.xloc S.xloc],[min(S.y(:,1,1)) max(S.y(:,1,1))],[S.zloc S.zloc],'k') % horizontal line 1
            plot3([min(S.x(1,:,1)) max(S.x(1,:,1))],[S.yloc S.yloc],[S.zloc S.zloc],'k') % horizontal line 2
        elseif S.cb_x.Value==1 && S.cb_y.Value==1 && S.cb_t.Value==0        % X Y
            if S.gain_cb.Value==1
        ...
        zlabel('z [m]')
        axis xy
        set(S.ax,'DataAspectratio',[1 1 1/S.aspval],'Zdir','normal')
        grid on
        axis tight
        set(S.ax,'Zlim',[min(S.z(:)) max(S.z(:))])
        % set colorscale
    ...
    function [] = slt_callback(varargin)
        ...
        set(findobj('Type','Figure','Name','3D-Block'), 'pointer', 'watch')
        S=guidata(gcbf);
        temp=S.z(1,1,min(abs(S.z(1,1,:)-S.slt.Value))==abs(S.z(1,1,:)-S.slt.Value));
        S.zloc=temp(1);
        S.text_tloc.String=num2str(S.zloc,'%8.2f');
        guidata(gcbf,S); % Update
        % call plot function
        plotnew(varargin);
    end
    ...
    function [] = tloc_edit(varargin)
        ...
        set(findobj('Type','Figure','Name','3D-Block'), 'pointer', 'watch')
        S=guidata(gcbf);
        temp=S.z(1,1,min(abs(S.z(1,1,:)-str2num(S.text_tloc.String)))==abs(S.z(1,1,:)-str2num(S.text_tloc.String)));
        S.zloc=temp(1);
        S.slt.Value=S.zloc;
        S.text_tloc.String=num2str(S.zloc,'%8.2f');
        guidata(gcbf,S); % Update
