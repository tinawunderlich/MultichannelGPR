function []=processingTestGUI_gssi(folder,name,profilelist,UTMzone)
...
[S.traces,S.dt,S.ns,S.x,S.y,S.z]=readGSSI(S.folder,UTMzone,S.profilelist(1));
...
S.fh.Visible='on';
    ...
    function S=addcomponents(S)
        ...
        S.asp_String=[{'10/1'} {'5/1'} {'4/1'} {'3/1'} {'2/1'} {'1/1'} {'1/2'} {'1/3'} {'1/4'} {'1/5'} {'1/10'} {'1/15'}];
        S.aspval=1/2;
...
%% choose new profile or channel
    function [] = profnum_call(varargin)
        ...
        [S.traces,S.dt,S.ns,S.x,S.y,S.z]=readGSSI(S.folder,UTMzone,S.profnum);
        S.raw=[];
        S.info=[];
        S.zz=[];
        for i=1:length(S.traces)
...
%% Apply processing steps
    function [] = apply_call(varargin)
        ...
        if S.t0corr.Value==1 && S.t0list.Value~=2 && S.t0list.Value~=3 %only for t0 with reference trace (4) und channelshift all profiles (1)
            ...
            [temptraces,~,~,~,~,ztemp]=readGSSI(S.folder,UTMzone,str2num(S.refprof.String));
            temp=[];
            infotemp=[];
            zztemp=[];
