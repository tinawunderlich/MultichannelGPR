function [] = Velocity_2D_picking_plot(foldername,name,profilelist)
...
S.profileinfo=temp.profileinfo;
S.m=matfile(fullfile(foldername,'profiles2mat','proc',[name,'_',int2str(profilelist(1)),'.mat']));
temp=load(fullfile(foldername,'profiles2mat',[name,'_',int2str(profilelist(1)),'_info.mat']));
S.info=temp.info;
S.traces=S.m.traces(:,(S.currchan-1)*S.profileinfo(S.profileinfo(:,1)==profilelist(1),5)+1:S.currchan*S.profileinfo(S.profileinfo(:,1)==profilelist(1),5));
S.dt=S.profileinfo(1,2);
S.ns=S.profileinfo(1,3);
S.chan=S.profileinfo(1,4);
S.x=S.info(4,S.info(3,:)==S.currchan)';
S.y=S.info(5,S.info(3,:)==S.currchan)';
...
uimenu(figureToolBar,'Label','Pan','Callback','pan on');
    ...
    function S=addcomponents(S)
        ...
        S.chan = uicontrol(S.fh,'style','listbox','unit','pix','position',[20 220 50 60],'callback',{@chan_call},'Value',1,'String',int2str([1:S.chan]'));
...
%         S.v=v(maxind(1))*100;  % best velocity in cm/ns
    ...
    function [] = profnum_call(varargin)
        ...
        drawnow;
        % read new data
        S.m=matfile(fullfile(S.foldername,'profiles2mat','proc',[S.name,'_',int2str(S.profnum),'.mat']));
        temp=load(fullfile(S.foldername,'profiles2mat',[S.name,'_',int2str(S.profnum),'_info.mat']));
        S.info=temp.info;
        S.traces=S.m.traces(:,(S.currchan-1)*S.profileinfo(S.profileinfo(:,1)==S.profnum,5)+1:S.currchan*S.profileinfo(S.profileinfo(:,1)==S.profnum,5));
        S.x=S.info(4,S.info(3,:)==S.currchan)';
        S.y=S.info(5,S.info(3,:)==S.currchan)';
        S.coord=cumsum([0; sqrt(diff(S.x).^2+diff(S.y).^2)]);    % profile coordinate
        S.dx=mean(sqrt(diff(S.x).^2+diff(S.y).^2));   % mean dx
