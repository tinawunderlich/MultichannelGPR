...
% writesegy(filename,data,varargin)
% Writes sgy-file
%
% Revision 1, 4-byte IEEE floating points, big endian
%
...
% 'dt', 'ns', 'cdpX', 'cdpY', 'SourceX', 'SourceY', 'GroupX',
% GroupY','gelev','selev','Inline3D','Crossline3D'
    ...
    h.dt=round(dt.*1e4);
    h.ns=ns;
    h.SegyFormatRevisionNumber=100; % Revision 1, 4-byte IEEE
    ...
    fwrite(segyfh,abs(h.dt),'uint16');                      % 3218
    fwrite(segyfh,abs(h.dtOrig),'uint16');                  % 3220
    ...
    % create trace header
    trh=createTraceheader(ns,abs(dt));
    ...
    for i=1:ntraces
        ...
        trh.cdpX = cdpX(i)*100;
        trh.cdpY = cdpY(i)*100;
        trh.SourceX=SourceX(i)*100;
        trh.SourceY = SourceY(i)*100;
        trh.GroupX=GroupX(i)*100;
        trh.GroupY = GroupY(i)*100;
        trh.ReceiverGroupElevation=gelev(i)*100;
        trh.SourceSurfaceElevation=selev(i)*100;
        trh.Inline3D=Inline3D;
        trh.Crossline3D=Crossline3D;
...
function trh=createTraceheader(ns,dt)
    ...
    trh.cdp=0;
    trh.cdpTrace=0;
    trh.TraceIdentificationCode=0;
    trh.NSummedTraces=0;
    trh.NStackedTraces=0;
    trh.DataUse=0;
    trh.offset=0;
    ...
    trh.ElevationScalar=-2;
    trh.SourceGroupScalar=-2; % divide coordinates by 100 to give meters!
    trh.SourceX=0;
    trh.SourceY=0;
    trh.GroupX=0;
    trh.GroupY=0;
